...
```{r project.setup, message = FALSE, error = FALSE, warning = FALSE}
#Import libraries
...
```{r first boxplot, echo = F, warning= FALSE, fig.align='left', fig.height=2.8, fig.cap= "2018 Ozone Concentration by Month"}
ozone_2018 <- epa_data %>% filter(Year == "2018")
ozone_2018$Month <- as.factor(ozone_2018$Month) 

ozone_2018_plot <-
  ggplot(ozone_2018, aes(x = factor(Month, level=1:12, labels=month.abb), y = Ozone)) +
  geom_boxplot() + 
  labs(x = "Month", y = "Ozone (ug/m^3)") + 
  theme_classic() 
print(ozone_2018_plot)
```

``` {r second boxplot, echo = F, warning=FALSE, fig.align='left', fig.height=2.8, fig.cap= "2019 Ozone Concentration by Month"}
ozone_2019 <- epa_data %>% filter(Year == "2019")
ozone_2019$Month <- as.factor(ozone_2019$Month) 

ozone_2019_plot <-
  ggplot(ozone_2019, aes(x = factor(Month, level=1:12, labels=month.abb), y = Ozone)) +
  geom_boxplot() + 
  labs(x = "Month", y = "Ozone (ug/m^3)") + 
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5))
print(ozone_2019_plot)
```
...
[My Github Repository.](https://github.com/anabishop99/EDA-Spring2023)
