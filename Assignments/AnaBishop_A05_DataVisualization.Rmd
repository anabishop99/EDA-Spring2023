...
```{r Create boxplots}
#5 
pp_chem_nutrients$month <- factor(pp_chem_nutrients$month, levels = month.abb) #changing months to their abbreviations

#temp boxplot
temp <- ggplot(pp_chem_nutrients, aes(x = month, y = temperature_C)) +
...
#phosphorus boxplot
tp <- ggplot(pp_chem_nutrients, aes(x = month, y = tp_ug)) +
  geom_boxplot(aes(color = lakename)) +
...
#nitrogen boxplot
tn <- ggplot(pp_chem_nutrients, aes(x = month, y = tn_ug)) +
  geom_boxplot(aes(color = lakename)) +
...
#cowplot
lake_characteristics <- plot_grid(temp, tp, tn, nrow = 3, align = 'h', rel_heights = c(3, 4, 3.5))
