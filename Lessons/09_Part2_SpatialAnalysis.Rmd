...
**NOTE: There is a big difference between mapView and ggplot. GGPlot can knit to a pdf, and mapView cannot b/c a pdf can't handle the interactiveness. However, you CAN export mapView as an HTML.
...
ggplot() +
  ...
  scale_fill_distiller(palette = "Y1ORd")
```
...
upperneuse <- huc8_sf %>% filter(SUBBASIN == "Upper Neuse")

mapView(huc8_sf, col.regions = "orange") +
  mapview(upperneuse, col.regions = "purple", alpha = 1)

ggplot() + 
  geom_sf(data = huc8_sf, fill = 'orange', color = 'gray') +
...
neuse_intersect_2 <- counties_sf_utm %>% 
  st_filter(neuse_sf, .pred = st_intersects)
