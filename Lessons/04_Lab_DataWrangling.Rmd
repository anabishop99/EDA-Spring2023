...
isolated_year <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

two_years <- filter(NTL.phys.data, lakename == "Tuesday Lake" & (year4 == 1990 | year4 == 1999))
range_years <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Because these can be more subjective than naming the actual queries that we want
