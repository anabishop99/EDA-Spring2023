...
## note - putting the aesthetics within the specific geom_point, etc lets you put multiple datasets into one plot. If you put it in the ggplot command then it will only apply that data to the plot
...
#What kind of values are months? - integer
class(PeterPaul.chem.nutrients$month)

#List unique values - 2, 5, 6, 7, 8, 9, 10, 11
unique(PeterPaul.chem.nutrients$month)

#Plot how many Lake observations occur each month - bar plot is good. A histogram would show the spread of the data as a continuous variable which isn't what we want - we want discreet observations by month. Bar plots work for every discrete/unique value
...
factor(PeterPaul.chem.nutrients$month, levels = 1:12)

#Try again: Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month, level=1:12, labels=month.abb))) + 
  geom_bar() + 
...
PeterPaul.chem.nutrients %>%
  filter(depth == 0) %>%
  ggplot(aes(y = temperature_C, x = daynum, color=year4)) + 
  geom_point(size=0.5, alpha=0.5) +
  facet_wrap(vars(lakename)) +
  labs(x = "Julian Day", y = "SST", color = "Year")
