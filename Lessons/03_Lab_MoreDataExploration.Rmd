...
vector1[3] # gives you the third object in the vector
...
matrix1[4, ] # pulls out the fourth row
matrix1[ , 3] # pulls out the 3rd column
matrix1[c(12, 14)] # gives us the 12th and 14th item from our matrix (by column)
matrix1[c(12:14)] # gives the 12th through 14th item in the matrix
matrix1[2:4, 1:3] # 
...
dataframe1[1:2,] # rows 1-2
dataframe1[c("ID","Passed")] # gives those two column names
dataframe1$ID
...
```{r DataExercise-1a}
high <- c(71, 64, 50, 43, 59, 60, 67, 67, 62, 52)
low <- c(58, 47, 37, 35, 40, 49, 48, 52, 32, 33)
```
...
```{r DataExercise-1c}
temps <- data.frame(high, low, high_c, low_c)
names(temps) <- c("High_F", "Low_F", "High_C", "Low_C")
...
```{r DataExercise-1d}
summary(temps)
summary(temps$High_F)
sd(temps$High_C) #sd won't work on the whole data frame, but will work on a specific vector/column
```
...
today <- Sys.Date()
format(today, format = "%B") # spelled out month
format(today, format = "%Y") # four digit year
format(today, format = "%y") # two digit year
#format(today, format = "")
...
year(date_obj_past) %% 100 # the percent-percent sign takes the remainder - gives 2 digit year
