...
# NOTE: summary is really only good to use after doing the join, because that is when you'll see any discontinuities in your data. If you're missing an obervation in the unprocessed data it won't show up
...
#shows seasonality b/c of up and down lines

ggplot(BR_complete, aes(x = Date, y = Temperature_C)) + 
  geom_line() +
  geom_smooth(method="lm")
#there is also a seasonal component here - same up and down pattern of the lines
```

There is a really small upward trend for Rainfall over time, but it is hardly noticeable at all. In contrast, the temperature has a more pronounced upward trend over the same time period, showing that temperature increased more quickly than rainfall over the study period.
...
#more general - these functions return the oldest year or date in the dataset
f_year <- year(first(BR_complete$Date))
f_month <- month(first(BR_complete$Date))

#using fyear and fmonth:
rainfall_ts <- ts(BR_complete$Rainfall, start = c(f_year, f_month), frequency = 12)
temp_ts <- ts(BR_complete$Temperature_C, start = c(f_year, f_month), frequency = 12)
```
...
#the large bar shows that you have to zoom into the smaller bar to see the variation in the data - trend vs. seasonal rows. Smaller bar = more significant impact on the data

#the temp trend does not increase linearly - it doesn't really do much for the first 50 years, then increases quickly
...
ggplot(rainfall_components, aes(x=Date)) +
  geom_line(aes(y=Observed))+
  geom_line(aes(y=trend, color = "red")) +
  ylab("Temperature")

ggplot(temp_components, aes(x=Date)) +
  geom_line(aes(y=Observed))+
  geom_line(aes(y=trend, color = "red")) +
  ylab("Temperature")
