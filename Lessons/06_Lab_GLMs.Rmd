...
```{r}
#Exercise 1:
# need to convert Year to a factor in order to use it as a distinguishing factor
ozone_density <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  geom_density(alpha = 0.5) + 
    scale_fill_manual(values=c("darkgreen", "darkblue")) +
  labs(x = "Ozone AQI Values", y = "Density", fill = "Year")

print(ozone_density)

ozone_boxplot <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 4, label = c("a", "b")) +
  labs(x = "Year", y = "Ozone AQI Values")

print(ozone_boxplot)

ozone_violin <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
...
test <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(test)
...
pm2.5_scatter <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) + 
  geom_point() + 
  geom_smooth(method = "lm", color="blue", se=FALSE) #removing standard error, aka the gray boundary
...
ozone_explanatories <- 
  ...
  drop_na() #drop_na eliminates all NAs overall, and na.omit eliminates NAs only in specific columns. However, in this case, both would have done the same thing

ozone_corr <- cor(ozone_explanatories)
corrplot(ozone_corr, method = "ellipse")
corrplot.mixed(ozone_corr, upper = "ellipse")
...
ozonebyALL.regression <- lm(data = ozone_explanatories, PM2.5 ~ Ozone + Year + Month + mean_Lng + mean_Lat)
step(ozonebyALL.regression)

# results show that you should remove Year and nothing else - that's what gives the lowest AIC
...
test_corrected <- lm(data = ozone_explanatories, PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat)
summary(test_corrected)
```
