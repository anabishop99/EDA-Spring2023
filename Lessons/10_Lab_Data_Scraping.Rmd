...
#change site no and period (24 days) IN the url
theURL_2 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data_2 <- read.table(theURL_2,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 

#Update the column headers
colnames(gage_data_2) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

#Tidy the data
gage_data_2 <- gage_data_2 %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data_2,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
## this package is specifically for NWIS water data only, so that's why you don't need to specify the URL you're pulling from
...
ggplot(wy_pop, aes(fill = estimate)) +
  geom_sf() +
  coord_sf(crs = 4326) +
  scale_fill_viridis(option = "viridis")
```
...
the_states <- html_nodes(the_website, 'td:nth-child(1)') %>% html_text()
the_price <- html_nodes(the_website, 'td:nth-child(2)') %>% html_text()
the_capacity <- html_nodes(the_website, 'td:nth-child(3)') %>% html_text()
net_generation <- html_nodes(the_website, 'td:nth-child(4)') %>% html_text()
total_retail <- html_nodes(the_website, 'td:nth-child(5)') %>% html_text()
  
#3 Construct a dataframe from the values
df_energy <- data.frame("State" = the_states,
